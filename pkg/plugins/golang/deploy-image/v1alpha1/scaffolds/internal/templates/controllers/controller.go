...
func (r *{{ .Resource.Kind }}Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	{{ lower .Resource.Kind }}.Status.Conditions = append({{ lower .Resource.Kind }}.Status.Conditions, metav1.Condition{Type: typeProgressing, Status: "True",Reason: "Reconciling", Message: "Starting reconciliation"})
	...
	if is{{ .Resource.Kind }}MarkedToBeDeleted {
		if controllerutil.ContainsFinalizer({{ lower .Resource.Kind }}, {{ lower .Resource.Kind }}Finalizer) {
			...
			{{ lower .Resource.Kind }}.Status.Conditions = append({{ lower .Resource.Kind }}.Status.Conditions, metav1.Condition{Type: typeDegraded,
	...
	if err != nil && apierrors.IsNotFound(err) {
		...
		if err != nil {
			...
			// The following implementation will update the status
			{{ lower .Resource.Kind }}.Status.Conditions = append({{ lower .Resource.Kind }}.Status.Conditions, metav1.Condition{Type: typeAvailable,
