...
func (r *{{ .Resource.Kind }}Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	{{ lower .Resource.Kind }}.SetCondition{{ .Resource.Kind }}(metav1.Condition{Type: typeProgressing{{ .Resource.Kind }}, Status: metav1.ConditionTrue, Reason: "Reconciling", Message: "Starting reconciliation"})
	...
	if is{{ .Resource.Kind }}MarkedToBeDeleted {
		if controllerutil.ContainsFinalizer({{ lower .Resource.Kind }}, {{ lower .Resource.Kind }}Finalizer) {
			...
			{{ lower .Resource.Kind }}.SetCondition{{ .Resource.Kind }}(metav1.Condition{Type: typeDegraded{{ .Resource.Kind }},
	...
	if err != nil && apierrors.IsNotFound(err) {
		...
		if err != nil {
			...
			// The following implementation will update the status
			{{ lower .Resource.Kind }}.SetCondition{{ .Resource.Kind }}(metav1.Condition{Type: typeAvailable{{ .Resource.Kind }},
