...
func (r *BusyboxReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	busybox.SetConditionBusybox(metav1.Condition{Type: typeProgressingBusybox, Status: metav1.ConditionTrue, Reason: "Reconciling", Message: "Starting reconciliation"})
	...
	if isBusyboxMarkedToBeDeleted {
		if controllerutil.ContainsFinalizer(busybox, busyboxFinalizer) {
			...
			busybox.SetConditionBusybox(metav1.Condition{Type: typeDegradedBusybox,
	...
	if err != nil && apierrors.IsNotFound(err) {
		...
		if err != nil {
			...
			// The following implementation will update the status
			busybox.SetConditionBusybox(metav1.Condition{Type: typeAvailableBusybox,
