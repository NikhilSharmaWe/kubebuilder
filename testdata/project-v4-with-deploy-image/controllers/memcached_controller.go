...
const (
	...
	typeProgressingMemcached = "Progressing"
...
func (r *MemcachedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	memcached.Status.Conditions = append(memcached.Status.Conditions, metav1.Condition{Type: typeProgressingMemcached, Status: "True", Reason: "Reconciling", Message: "Starting reconciliation"})
	...
	if isMemcachedMarkedToBeDeleted {
		if controllerutil.ContainsFinalizer(memcached, memcachedFinalizer) {
			...
			memcached.Status.Conditions = append(memcached.Status.Conditions, metav1.Condition{Type: typeDegradedMemcached,
	...
	if err != nil && apierrors.IsNotFound(err) {
		...
		if err != nil {
			...
			// The following implementation will update the status
			memcached.Status.Conditions = append(memcached.Status.Conditions, metav1.Condition{Type: typeAvailableMemcached,
	...
	memcached.Status.Conditions = append(memcached.Status.Conditions, metav1.Condition{Type: typeAvailableMemcached,
		...
		Message: fmt.Sprintf("Deployment for custom resource (%s) created succefully", memcached.Name)})
	if err := r.Status().Update(ctx, memcached); err != nil {
		log.Error(err, "Failed to update Memcached status")
		return ctrl.Result{}, err
	}
