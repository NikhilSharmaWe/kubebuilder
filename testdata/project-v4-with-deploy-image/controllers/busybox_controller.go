...
const (
	...
	typeProgressingBusybox = "Progressing"
...
func (r *BusyboxReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	busybox.Status.Conditions = append(busybox.Status.Conditions, metav1.Condition{Type: typeProgressingBusybox, Status: "True", Reason: "Reconciling", Message: "Starting reconciliation"})
	...
	if isBusyboxMarkedToBeDeleted {
		if controllerutil.ContainsFinalizer(busybox, busyboxFinalizer) {
			// Run finalization logic for busyboxFinalizer. If the
			// finalization logic fails, don't remove the finalizer so
			// that we can retry during the next reconciliation.
			log.Info("Performing Finalizer Operations for Busybox before delete CR")

			// The following implementation will update the status
			busybox.Status.Conditions = append(busybox.Status.Conditions, metav1.Condition{Type: typeDegradedBusybox,
			...
			// Remove busyboxFinalizer. Once all finalizers have been
	...
	if err != nil && apierrors.IsNotFound(err) {
		...
		if err != nil {
			...
			// The following implementation will update the status
			busybox.Status.Conditions = append(busybox.Status.Conditions, metav1.Condition{Type: typeAvailableBusybox,
